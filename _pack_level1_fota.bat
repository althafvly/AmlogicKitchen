...
bin\img2sdat tmp\odm.img -o tmp -v 4 -p odm
bin\brotli.exe --in tmp\odm.new.dat --out tmp\odm.new.dat.br --quality 6 -w 24
del tmp\odm.img tmp\odm.new.dat

copy level1\product.PARTITION tmp\product.img
bin\img2sdat tmp\product.img -o tmp -v 4 -p product
bin\brotli.exe --in tmp\product.new.dat --out tmp\product.new.dat.br --quality 6 -w 24
del tmp\product.img tmp\product.new.dat

copy level1\vendor.PARTITION tmp\vendor.img
bin\img2sdat tmp\vendor.img -o tmp -v 4 -p vendor
echo Please Wait...
bin\brotli.exe --in tmp\vendor.new.dat --out tmp\vendor.new.dat.br --quality 6 -w 24
del tmp\vendor.img tmp\vendor.new.dat

copy level1\system.PARTITION tmp\system.img
bin\img2sdat tmp\system.img -o tmp -v 4 -p system
echo Please Wait...
bin\brotli.exe --in tmp\system.new.dat --out tmp\system.new.dat.br --quality 6 -w 24
del tmp\system.img tmp\system.new.dat

bin\7za a out\update_tmp.zip .\tmp\*

echo Signing...
java -jar bin\zipsigner.jar out\update_tmp.zip "out\%filename%_fota.zip"
