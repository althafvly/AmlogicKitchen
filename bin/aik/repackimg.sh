...
case $1 in
--help)
  echo "usage: repackimg.sh [--local] [--original] [--origsize] [--level <0-9>] [--avbkey <name>] [--forceelf]"
  exit 1
  ;;
esac
...
if [ "$original" ]; then
  ...
  lz4-l)
    repackcmd="$bin/$arch/lz4 $level -l"
    compext=lz4
    ;;
  cpio)
    repackcmd="cat"
...
if [ "$original" ]; then
  ramdisk=$(ls *-*ramdisk.cpio*)
  echo "${vendor}ramdisk = $ramdisk"
...
*)
  ...
  if [ -f *-recovery_dtbo ]; then
    recoverydtbo=$(ls *-recovery_dtbo)
    echo "recovery_dtbo = $recoverydtbo"
