...
case $1 in
--help)
  echo "usage: unpackimg.sh [--local] [--nosudo] <file>"
  exit 1
  ;;
--local)
  local="--local"
...
if [ -d split_img -o -d ramdisk ]; then
  if [ -d ramdisk ] && [ "$(stat $statarg ramdisk | head -n 1)" = "root" -o ! "$(
    find ramdisk 2>&1 | cpio -o >/dev/null 2>&1
    echo $?
  )" -eq "0" ]; then
    rmsumsg=" (as root)"
  fi
