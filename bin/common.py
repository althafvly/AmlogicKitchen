...
def LoadInfoDict(input_file, input_dir=None):
    ...
    try:
        d = LoadDictionaryFromLines(
            read_helper("META/misc_info.txt").split("\n"))
    except KeyError:
    ...
    if "tool_extensions" not in d:
        try:
            d["tool_extensions"] = read_helper(
                "META/tool-extensions.txt").strip()
        except KeyError:
    ...
    if fstab_version == 1:
        ...
        for line in data.split("\n"):
            ...
            if not 3 <= len(pieces) <= 4:
                raise ValueError(
                    "malformed recovery.fstab line: \"%s\"" % (line,))
            options = None
            if len(pieces) >= 4:
...
def _BuildBootableImage(sourcedir, fs_config_file, info_dict=None,
    ...
    def make_ramdisk():
        ...
        p2 = Run(["minigzip"], stdin=p1.stdout,
...
def SignFile(input_name, output_name, key, password, min_api_level=None,
    ...
    if p.returncode != 0:
        raise ExternalError(
            "signapk.jar failed: return code %s" % (p.returncode,))
...
class PasswordManager(object):
    ...
    def GetPasswords(self, items):
        ...
        while True:
            ...
            if not first:
                print("key file %s still missing some passwords." %
                      (self.pwfile,))
...
class BlockDifference(object):
    ...
    def WriteScript(self, script, output_zip, progress=None):
        if not self.src:
            ...
            script.Print("Patching %s image unconditionally..." %
                         (self.partition,))
        else:
            script.Print("Patching %s image after verification." %
                         (self.partition,))
    ...
    def WriteVerifyScript(self, script, touched_blocks_only=False):
        ...
        # full OTA
        if not self.src:
            script.Print("Image %s will be patched unconditionally." %
                         (partition,))
        ...
        # incremental OTA
            ...
            if self.version >= 4:
                ...
                if self.check_first_block:
                    script.AppendExtra(
                        'check_first_block("%s");' % (self.device,))
...
def MakeRecoveryPatch(input_dir, output_sink, recovery_img, boot_img,
    ...
    if full_recovery_image:
        ...
        path = os.path.join(input_dir, "SYSTEM", "etc",
...
""" % {'boot_size': boot_img.size,
    ...
    for init_rc_file in init_rc_files:
        ...
        with open(os.path.join(init_rc_dir, init_rc_file)) as f:
            for line in f:
                m = re.match(
                    r"^service flash_recovery /system/(\S+)\s*$", line)
                if m:
