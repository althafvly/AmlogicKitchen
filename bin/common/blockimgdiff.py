...
class BlockImageDiff(object):
        ...
        for xf in self.transfers:
            ...
            if self.version == 1:
                ...
                if unstashed_src_ranges.size() > 0:
                    ...
                    if xf.use_stash:
                        mapped_unstashed = xf.src_ranges.map_within(
            ...
            if xf.style == "diff" and self.version >= 3:
                ...
                if xf.src_ranges.overlaps(xf.tgt_ranges):
                    if stashed_blocks + xf.src_ranges.size() > max_allowed:
                        ...
                        print("%10d  %9s  %s" %
                              (xf.src_ranges.size(), "implicit", xf))
    ...
    def ComputePatches(self, prefix):
        ...
        if diff_q:
            if self.threads > 1:
                print("Computing patches (using %d threads)..." %
                      (self.threads,))
            else:
            ...
            def diff_worker():
                while True:
                    ...
                    patch = compute_patch(
                        src, tgt, imgdiff=(xf.style == "imgdiff"))
                    size = len(patch)
                    with lock:
    ...
    def FindTransfers(self):
        ...
        def AddTransfer(tgt_name, src_name, tgt_ranges, src_ranges, style, by_id,
            ...
            if style != "diff" or not split:
                Transfer(tgt_name, src_name, tgt_ranges,
        ...
        for tgt_fn, tgt_ranges in self.tgt.file_map.items():
            if tgt_fn == "__ZERO":
                ...
                AddTransfer(tgt_fn, None, tgt_ranges,
                            empty, "new", self.transfers)
                continue
